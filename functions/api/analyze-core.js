...
export async function onRequestPost(context) {
  ...
  return json({ rows: finalRows, warnings, telemetry, rulesVersion: rulesPayload.version }, 200, { 'cache-control': 'no-store' });
}
...
function callTelemetry(response, { model, reasoning, stage, useWeb }) {
  ...
  return { stage, model, reasoning, inputTokens, cachedInputTokens, outputTokens, reasoningTokens, webSearchCalls, estimatedUsd, cacheWriteCeilingUsd };
}
